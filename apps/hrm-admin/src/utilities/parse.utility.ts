...
export const isInt = (n: any) => {
  ...
  return (
    Number.isInteger(n) || Math.floor(Number(n)) === n || Number(n) % 1 === 0
  );
};
...
export const convertArrayToObject = (arr: any[], keyMap: string) => {
  if (!arr.length) {
    return {};
  }

  return arr.reduce((final, data) => {
    return {
      ...final,
      [data[keyMap]]: { ...data },
    };
